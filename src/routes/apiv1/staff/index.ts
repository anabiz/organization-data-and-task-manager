...
import { getAllStaffByCompanyId, getStaffById, deleteStaffById, updateStaffById } from "../../../controller/queries";
...
router.post("/addstaff", checkCeo, async function (req: Request, res: Response) {
  try {
    const data = req.body;
...
router.get("/allstaff", verifyToken, async function (req: Request, res: Response) {
...
router.get("/delete/:id", checkCeo, async function (req: Request, res: Response) {
  const staffId = req.params.id;
  try {
    console.log("i am good to go");
    ...
    return res.status(200).json({ staffData: staff, successMessage: "account deleted" })

  } catch (error) {
    return res.status(200).json({ data: "unable to delete staff account try again later" });
  }
});

//Only a staff can update his account, even the ceo can not update staff account. 
router.put("/update/:id", checkStaffAuth, async function (req: Request, res: Response) {
